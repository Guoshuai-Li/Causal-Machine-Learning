...
- X's past values contain significant information for predicting Y's future values
- Y's past values do NOT improve predictions of X's future values beyond X's own history
- This is exactly what we expect from the DGP: Y = f(Y_past, X_past) but X = f(X_past only)

**Important caveats**:
- **Granger causality ≠ True causality**: Granger causality only measures predictive relationships
- **Linear assumption**: The test assumes linear relationships (our DGP is linear)
- **Stationarity requirement**: Results are valid because both series are stationary
- **No hidden confounders**: In controlled simulations, we know there are no confounding variables

## Core Theory

### Granger Causality Test Mechanism

For testing if X Granger-causes Y:

**Restricted Model** (Null Hypothesis H₀):
```
Y(t) = α₀ + α₁Y(t-1) + ... + αₖY(t-k) + ε
```

**Unrestricted Model** (Alternative Hypothesis H₁):
```
Y(t) = α₀ + α₁Y(t-1) + ... + αₖY(t-k) + β₁X(t-1) + ... + βₖX(t-k) + ε
```
...
The Augmented Dickey-Fuller test examines:

**Test Equation**:
```
Δy(t) = α + βt + γy(t-1) + Σδᵢ Δy(t-i) + ε
```

**Hypotheses**:
- H₀: γ = 0 (series has unit root, non-stationary)
- H₁: γ < 0 (series is stationary)

**Interpretation**:
- More negative ADF statistic → stronger evidence for stationarity
- If ADF statistic < critical value (e.g., -3.43 at 1%) → reject H₀ → stationary

## Conclusion

This experiment successfully demonstrates:

✅ **Correct identification of causal direction**: X → Y detected, Y → X rejected

✅ **Validation of DGP**: Results align perfectly with the underlying data generation process

✅ **Understanding of methodology**: 
- Stationarity testing as prerequisite
- Granger causality testing in both directions
- Interpretation of F-statistics and p-values

✅ **Foundation for advanced research**: This stationary case provides the baseline for studying causal discovery under non-stationarity

## Next Steps

For future exploration:

1. **Experiment with different DGP parameters** to see how causality strength affects test statistics
2. **Add more variables** to create multivariate systems
3. **Introduce non-stationarity** (e.g., time-varying parameters, structural breaks)
4. **Compare with other methods** (e.g., ICP, CCM, transfer entropy)
5. **Test on real-world data** where true causality is unknown
...
This foundational experiment in **stationary Granger causality** provides essential baseline knowledge for advanced research in:

- **Causal mechanism discovery under non-stationarity**
- **Invariant Causal Prediction (ICP)** in changing environments
- **Temporal causality with distributional shifts**
- **Robust causal identification across different regimes**

Understanding the stationary case is crucial before tackling the complexities introduced by non-stationarity in real-world systems.
